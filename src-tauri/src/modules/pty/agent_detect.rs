...
const arnavterminal_MARKER: &[u8] = b"notify;arnavterminal;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(event) = pt.strip_prefix(arnavterminal_MARKER) {
...
mod tests {
    ...
    #[test]
    fn arnavterminal_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;arnavterminal;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;arnavterminal;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;arnavterminal;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;arnavterminal;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn arnavterminal_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;arnavterminal;attention")),
