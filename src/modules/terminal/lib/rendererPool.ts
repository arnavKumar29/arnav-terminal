...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-arnavterminal-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-arnavterminal-slot", String(slots.length));
...
function bindSlot(slot: Slot, p: AcquireParams): void {
  ...
  if (p.snapshot) {
    try {
      ...
      console.warn("[arnavterminal] snapshot replay failed:", e);
    }
  }
  if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[arnavterminal] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[arnavterminal-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[arnavterminal-webgl] dispose failed:", e);
  }
  try {
    const r = (
