...
const INIT_PROMPT = `Scan this workspace and produce arnavterminal.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap arnavterminal.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const arnavterminal_CMD_RE =
  /^<arnavterminal-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<arnavterminal-command name="${name}" />\n\n${prompt}`;
}
