...
const arnavterminal_MD_MAX_BYTES = 32 * 1024;
type MemoryCacheEntry = { content: string | null; mtime: number };
const projectMemoryCache = new Map<string, MemoryCacheEntry>();

async function readarnavterminalMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/arnavterminal.md`;
  ...
  try {
    ...
    const content =
      r.content.length > arnavterminal_MD_MAX_BYTES
        ? r.content.slice(0, arnavterminal_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readarnavterminalMd(live.workspaceRoot);
