...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("arnavterminal:ai-attach-file", onAttach);
    return () => window.removeEventListener("arnavterminal:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the arnavterminal.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<arnavterminal-command name="${outcome.commandName}" />`;
        }
      }
    }
