...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "arnavterminal needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "arnavterminal run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "arnavterminal finished", "Your task is ready");
    }
  }, [status, error]);
