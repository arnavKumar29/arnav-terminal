...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S arnavterminal-bin";
    case "debian":
      return `sudo apt install ./arnavterminal_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./arnavterminal-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `arnavterminal v${manual.version} is available`
                  : `arnavterminal v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart arnavterminal to finish installing."
              : downloading
                ? progress !== null
            ...
            <div className="flex gap-1 rounded-md bg-muted/40 p-1">
              {DISTROS.map((d) => (
                <button
                  ...
                  className={`flex-1 rounded px-2 py-1 text-[11px] transition-colors ${distro === d.key
